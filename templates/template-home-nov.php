					...
					<div class="container">
						 <?php if ( get_sub_field ( 'feature_image_or_infogram' ) == 'image' ) { ?>
							 <div class="featureBlock">
								 <?php
					$inline_img_147_src = get_sub_field( 'image' );
					$inline_img_147_attach_id = $inline_img_147_src ? attachment_url_to_postid( $inline_img_147_src ) : 0;
					if ( $inline_img_147_attach_id ) {
						echo wp_get_attachment_image( $inline_img_147_attach_id, 'full', false, array( 'alt' => '', 'class' => 'featureImage' ) );
					} elseif ( $inline_img_147_src ) {
						echo '<img class="featureImage" src="' . esc_url( $inline_img_147_src ) . '" loading="lazy" alt="' . esc_attr( '' ) . '" />';
					}
				?>
							 </div>
						 <?php } else { ?>
