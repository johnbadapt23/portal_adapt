...
function mepr_update_monthly_count($user_id, $meta_key_array, $meta_key_total) {
    ...
    $new_data = array_filter($data, function($entry) use ($twelve_months_ago) {
        return $entry['month'] >= $twelve_months_ago;
    });
...
function mepr_trim_last_year_month($user_id, $meta_key_array, $meta_key_total) {
    if (date('d') !== '01') return; // Only run on the 1st day of the month
...
function update_user_activity_info() {
    ...
    foreach ($users as $user) {
        ...
        // --- Trim last year's same month if it's the first day ---
        ...
        if (!get_user_meta($user_id, 'mepr_backfill_done', true)) {
            if ($login_count_thirty > 0) {
                mepr_add_to_monthly_count($user_id, 'mepr_logins_12_months_array', 'mepr_logins_twelve_months', $login_count_thirty);
            }
            if ($download_count_thirty > 0) {
                mepr_add_to_monthly_count($user_id, 'mepr_downloads_12_months_array', 'mepr_downloads_twelve_months', $download_count_thirty);
            }
            if ($post_count_thirty > 0) {
                mepr_add_to_monthly_count($user_id, 'mepr_post_views_12_months_array', 'mepr_post_views_twelve_months', $post_count_thirty);
            }

            // ✅ Mark as completed for this user so it never runs again
            update_user_meta($user_id, 'mepr_backfill_done', 1);
        }
