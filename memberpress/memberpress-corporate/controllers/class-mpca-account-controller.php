...
class MPCA_Account_Controller {
  ...
  public function manage_sub_accounts($action) {
    if($action == 'manage_sub_accounts') {
      ...
      $mepr_company_name = $ca_owner->mepr_company_name;
      
      $mepr_current_user_company = $mepr_current_user->mepr_company_name;
      $owner_name = empty($full_name) ? $ca_owner->user_login : $full_name;
      if (isset($mepr_company_name) && isset($ca_owner->user_login)) {
        $owner_company = empty($mepr_company_name) ? $ca_owner->user_login : $mepr_company_name;
      } 
      if (isset($mepr_current_user_company) && isset($ca_owner->user_login)) {
        $owner_company = empty($mepr_current_user_company) ? $ca_owner->user_login : $mepr_current_user_company;
      }       
