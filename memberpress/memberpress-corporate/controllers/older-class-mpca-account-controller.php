...
class MPCA_Account_Controller {
  ...
  public function maybe_add_sub_account_management_link($user, $row, $transaction, $issub) {
    ...
    if(!empty($ca) && isset($ca->id) && !empty($ca->id)) {
      ?>
      <a href="<?php echo $ca->sub_account_management_url(); ?>" class="mepr-account-row-action mepr-account-manage-sub-accounts"><?php _e('Set up your team', 'memberpress-corporate'); ?></a>
      <?php
    }
  }
