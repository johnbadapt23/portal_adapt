...
<div class="mp_wrapper">
  <form class="mepr-signup-form mepr-form" method="post" action="<?php echo $_SERVER['REQUEST_URI'].'#mepr_jump'; ?>" enctype="multipart/form-data" novalidate>
    ...
    <span class="section-details">
        <h4 class="form-title">My Details</h4>
        <p>We'll use your work details to set up your access.</p>
    </span>
    <?php if((!MeprUtils::is_user_logged_in() ||
      ...
      <div class="mp-form-row mepr_first_name">
        ...
        <input type="text" name="user_first_name" placeholder="First Name" id="user_first_name<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo esc_attr($first_name_value); ?>" <?php echo ($mepr_options->require_fname_lname)?'required':''; ?> />
      </div>
      <div class="mp-form-row mepr_last_name">
        ...
        <input type="text" name="user_last_name" placeholder="Last Name" id="user_last_name<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo esc_attr($last_name_value); ?>" <?php echo ($mepr_options->require_fname_lname)?'required':''; ?> />
      </div>
    ...
    <?php else: ?>
      ...
      <div class="mp-form-row mepr_email">
        <div class="mp-form-label">
          <label for="user_email<?php echo $unique_suffix; ?>"><?php _ex('Work email address:*', 'ui', 'memberpress'); ?></label>
          <span class="cc-error"><?php _ex('Invalid Email', 'ui', 'memberpress'); ?></span>
        </div>
        <input type="email" name="user_email" placeholder="Work email address" id="user_email<?php echo $unique_suffix; ?>" class="mepr-form-input" value="<?php echo (isset($user_email))?esc_attr(stripslashes($user_email)):''; ?>" required />
      </div>
      ...
      <span class="section-details custom-fields-section">
          <h4 class="form-title topics-form-title">What are you interested in?</h4>
          <p class="topics-form-text">This will help us point you in the right direction. Choose at least three topics</p>
      ...
      <span class="section-details">
          ...
          <p>Choose a password with at least <strong>8 characters</strong> that contains a <strong>symbol or a number</strong></p>
      </span>
      <?php if($mepr_options->disable_checkout_password_fields === false): ?>
        <div class="mp-form-row mepr_password">
          <input type="password" name="mepr_user_password" id="mepr_user_password<?php echo $unique_suffix; ?>" placeholder="Password" class="mepr-form-input mepr-password" value="<?php echo (isset($mepr_user_password))?esc_attr(stripslashes($mepr_user_password)):''; ?>" required />
          <div class="mp-form-label">
    ...
    <div class="mp-form-submit">
      ...
      <input type="submit" class="mepr-submit" value="Create my account" />
...
<script type="text/javascript">
    jQuery('.mepr_password img.show-password').on('click', function(e){
        var x = document.getElementById("mepr_user_password<?php echo $unique_suffix; ?>");
        if (x.type === "password") {
            x.type = "text";
    ...
    jQuery('.mepr_password_confirm img.show-password').on('click', function(e){
        var x = document.getElementById("mepr_user_password_confirm<?php echo $unique_suffix; ?>");
        if (x.type === "password") {
            x.type = "text";
            jQuery(this).addClass('active');
        } else {
    ...
    jQuery('.mepr_mepr_company_name input').attr('placeholder', 'Company Name');
    jQuery('.mepr_mepr_job_title input').attr('placeholder', 'Job Title');
</script>
