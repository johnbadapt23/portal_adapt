...
class MeprAppHelper {
  ...
  public static function format_price_string($obj, $price = 0.00, $show_symbol = true, $coupon_code = null, $show_prorated = true) {
    ...
    if(!empty($coupon)) { $price_str = sprintf(__('PAID with coupon %s','memberpress'), $coupon_code); }
  ...
  public static function clipboard_input($value, $input_classes='', $icon_classes='') {
    ?>
      <input type="text" class="mepr-clipboard-input <?php echo $input_classes; ?>" onfocus="this.select();" onclick="this.select();" readonly="true" value="<?php echo $value; ?>" />
